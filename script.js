...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // ============================================
  // SUBTLE GLITCH AUDIO EFFECT FOR AUDIOBOOK
  // ============================================
  let audioContext = null;
  let audiobookSource = null;
  let noiseGainNode = null;
  let glitchInitialized = false;
  const NOISE_LEVEL = 0.05; // 5% noise level

  function initAudiobookGlitch() {
    if (glitchInitialized || !audiobookAudio) return;

    try {
      audioContext = new (window.AudioContext || window.webkitAudioContext)();
      audiobookSource = audioContext.createMediaElementSource(audiobookAudio);

      // Create a subtle noise/crackle effect
      noiseGainNode = audioContext.createGain();
      noiseGainNode.gain.value = NOISE_LEVEL;

      // Create noise buffer (vinyl crackle style)
      const noiseBuffer = audioContext.createBuffer(1, audioContext.sampleRate * 2, audioContext.sampleRate);
      const noiseData = noiseBuffer.getChannelData(0);
      for (let i = 0; i < noiseData.length; i++) {
        // Sparse crackle - mostly silence with occasional pops
        noiseData[i] = Math.random() > 0.997 ? (Math.random() * 2 - 1) * 0.5 : (Math.random() * 2 - 1) * 0.02;
      }
      ...
      // Very subtle low-pass filter to warm the crackle
      const noiseFilter = audioContext.createBiquadFilter();
      noiseFilter.type = 'lowpass';
      noiseFilter.frequency.value = 3000;

      // Subtle bitcrusher-like effect via waveshaper (very mild)
      const waveshaper = audioContext.createWaveShaper();
      const curve = new Float32Array(256);
      for (let i = 0; i < 256; i++) {
        const x = (i / 128) - 1;
        // Very subtle saturation - barely noticeable
        curve[i] = x * (1 + 0.02 * Math.abs(x));
      }
      waveshaper.curve = curve;
      waveshaper.oversample = '2x';

      // Main audio path with subtle warmth
      audiobookSource.connect(waveshaper);
      waveshaper.connect(audioContext.destination);

      // Noise path (very quiet crackle layer)
  ...
  if (audiobookAudio) {
    audiobookAudio.addEventListener('play', () => {
      ...
      // Turn noise back on when playing
      if (noiseGainNode) {
        noiseGainNode.gain.value = NOISE_LEVEL;
      }
    });
    ...
    audiobookAudio.addEventListener('pause', () => {
      if (noiseGainNode) {
